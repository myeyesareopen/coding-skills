...
- 预计较长的构建、测试或运行日志写入任务独立的 `work/` 子目录；工具返回命令、实际退出码、结果摘要、关键错误和证据路径。短输出直接返回，不为每个命令额外创建日志或摘要调用。敏感值先脱敏，长日志不进入交付文件或 Git 提交。
...
- 按调用设置够用的输出上限，优先在命令端筛选。测试通过返回简短统计和代码状态；失败先读取首个有意义的错误及相关堆栈，按需要扩展。详细证据留在文件，不依靠截断后的聊天记录恢复。
