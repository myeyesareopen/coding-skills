...
仅用户明确要求 DeepSeek 时使用本流程；纯 native Luna/Sol/Astra 任务不安装、不配置 key、不启动 DeepSeek。主代理负责安装、凭据配置和后台进程，实施者不处理这些环境决策。本流程供所选 DeepSeek 路径首次使用或环境故障时执行。正常可用的环境直接复用，不重复安装或新增常驻服务。
...
若回退时仍有其他 legacy DeepSeek 执行者运行，Luna 纳入混合共享容量；dsh/key 缺失不影响 `dispatch.mjs aux`，可正常预留/释放名额。混合执行且 Node/runner 本身不可用时，主代理核对共享状态与已知进程占用，确认余量后保守串行运行一个 Luna；不能删锁绕过上限。确认本主代理的 DeepSeek 执行者全部结束、后续只用 native 后，恢复平台容量管理，不再新增 aux 预留；已有预留在对应 native 代理结束后显式释放。保留其他主代理及未知占用。
